...
# ---------------------------------------------------------------------------
# Tapping season (Kerala rubber)
# Season runs June through February; March–May is off-season (hot, dry period
# before the south-west monsoon arrives and trees need rest).
# ---------------------------------------------------------------------------
TAPPING_SEASON_MONTHS = list(range(6, 13)) + list(range(1, 3))  # Jun–Dec + Jan–Feb

# ---------------------------------------------------------------------------
# Yield & labour constants (domain knowledge from practitioners)
# ---------------------------------------------------------------------------
TREES_PER_ACRE = 200              # Standard planting density in Kerala
TREES_PER_TAPPER_BLOCK = 300     # One tapper handles one block of 300 trees per day
LITRES_PER_BLOCK_PER_DAY = 50    # Typical latex yield per 300-tree block on a good day
MIN_TREES_FOR_ONE_TAPPER = 600   # Minimum viable farm for one full-time tapper (2 blocks)

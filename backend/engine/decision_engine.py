...
def _compute_yield_estimate(profile: PlantationProfile, today: Optional[date] = None) -> YieldEstimate:
    """
    Compute estimated latex yield and labour requirements from plantation profile.
    Uses domain constants: 200 trees/acre, 300 trees/block, 50 L/block/day.
    """
    off_season = False
    off_season_note = None
    if today is not None and today.month not in TAPPING_SEASON_MONTHS:
        off_season = True
        off_season_note = (
            f"You are currently outside the Kerala rubber tapping season (June–February). "
            f"Tapping typically resumes in June when the monsoon begins and the trees recover from the hot dry period."
        )

    if not profile.num_trees:
        return YieldEstimate(off_season=off_season, off_season_note=off_season_note)

    num_trees = profile.num_trees
    num_blocks = max(1, round(num_trees / TREES_PER_TAPPER_BLOCK))
    estimated_litres = num_blocks * LITRES_PER_BLOCK_PER_DAY
    tappers_needed = num_blocks  # one tapper per block per day
    size_acres = round(num_trees / TREES_PER_ACRE, 1)

    # Build plain-language note
    parts = [
        f"Your {num_trees:,} trees form approx. {num_blocks} tapper block{'s' if num_blocks != 1 else ''} "
        f"({TREES_PER_TAPPER_BLOCK} trees each)."
    ]
    parts.append(
        f"Expected yield on a good tapping day: ~{int(estimated_litres)} litres of latex "
        f"({LITRES_PER_BLOCK_PER_DAY} L/block)."
    )
    if tappers_needed == 1:
        parts.append("One tapper can manage your full plantation per day.")
    else:
        parts.append(f"{tappers_needed} tappers would be needed to cover all blocks in one day.")
    if num_trees < MIN_TREES_FOR_ONE_TAPPER:
        parts.append(
            f"With fewer than {MIN_TREES_FOR_ONE_TAPPER} trees ({MIN_TREES_FOR_ONE_TAPPER // TREES_PER_ACRE} acres), "
            "most small farmers tap their own land without hiring a tapper."
        )

    # Latex sale method context
    if profile.latex_sale_method == LatexSaleMethod.liquid_latex:
        parts.append(
            "Selling as liquid latex — add acid to the collection drum to prevent premature solidification. "
            "A lab test will determine the Dry Rubber Content (DRC) for pricing."
        )
    elif profile.latex_sale_method == LatexSaleMethod.rubber_sheets:
        parts.append(
            "Processing into rubber sheets — pour latex into trays, add coagulating acid, pass through rollers, "
            "then sun-dry before smoke-curing. Wet weather delays the drying step."
        )

    return YieldEstimate(
        num_blocks=num_blocks,
        estimated_litres=float(estimated_litres),
        tappers_needed=tappers_needed,
...
def evaluate(request: DecisionRequest) -> DecisionResponse:
    ...
    # Compute yield estimate (includes seasonal awareness)
    yield_estimate = _compute_yield_estimate(profile, target_date)
    ...
    # --- Off-season warning ---
    if yield_estimate.off_season and yield_estimate.off_season_note:
        reasoning.append(f"Off-season notice: {yield_estimate.off_season_note}")
